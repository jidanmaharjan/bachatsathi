...
const History = () => {
  ...
  const { isAuthenticated, isLoading :userisLoading } = useSelector((state)=> state.user)
  ...
  useEffect(()=>{
    const selectmonth = []
    const selectyear = []
    allMonth && allMonth.length>0 && allMonth.map(unit=>{
    ...
    <div className=" p-4 mt-4 mb-16 sm:mb-0">
      ...
      <div className="p-4 rounded-md bg-gray-100 mb-4">
        {loading ? 'loading' : (
          ...
          <select value={year} onChange={(e)=>setYear(e.target.value)} className='p-2 outline-none sm:w-40 bg-inherit border border-gray-300 mr-2 cursor-pointer rounded-md'>
            {selectYearData.length>0 && selectYearData.map(y =>(
              <option key={y} value={y}>{moment(y).format('YYYY')}</option>
            ))}
          </select>
          <select value={month} onChange={(e)=>setMonth(e.target.value)} className='p-2 outline-none sm:w-40 bg-inherit border border-gray-300 mx-2 cursor-pointer rounded-md'>
          {selectMonthData.length>0 && selectMonthData.map(m =>(
              <option key={m} value={m}>{moment(m).format('MMMM')}</option>
            ))}
          </select>
      ...
      <div className='p-4 rounded-md bg-gray-100 mb-4 transition-all ease-in-out duration-300'>
        {allMonth && allMonth.length > 0 && allMonth.filter(fmonth=>fmonth && fmonth.date && fmonth.date === `${month} ${year}`).map(final=>(
          <div key={final.date} className='transition-all ease-in-out duration-300'>
            <p className='text-lg font-semibold text-blue-400'>{moment(final.date.split(' ')[1]+final.date.split(' ')[0]).format('MMMM YYYY')}</p>
            ...
            <table className="w-full mt-2 rounded-md overflow-hidden ">
              ...
              <tbody className="text-gray-700 text-sm sm:text-md 2xl:text-lg">
                {final.collected && final.collected.map((member, index) =>(
              ...
              <td className="p-2"><p>{member.amount  }</p></td>
              <td className="p-2"><p>{member.fine  }</p></td>
                </tr>
                ))}
